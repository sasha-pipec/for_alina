...
from collection_app.models import Tracks, Artists, Genres, Albums
import json
...
class ViewSetsTests(TestCase):
    id_query = '?id='
    pages = (
        (Genres, '/rest/Genres/', {'title': 'genre'}, {'description': 'new_description'}),
        (Tracks, '/rest/Tracks/', {'title': 'track', 'year': 2000, 'rating': 6}, {'rating': 5}), # в attr не прокинула rating
        ...
        #(Client, '/rest/Client/', {'user': 'username'}, {'money': 100}) --> Нету такого эндпоинта
    )

    def setUp(self):
        # self.client = Client() --> не нужно переопределять client, он и так есть
